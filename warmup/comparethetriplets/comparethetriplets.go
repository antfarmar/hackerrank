...
var (
	//fileIn        = os.Stdin
	testData        = `C:\Users\maf\go\src\hackerrank\warmup\comparethetriplets\compare-the-triplets-testcases\input\input00.txt`
...
func main() {
	...
	A := atois(readfields())
	B := atois(readfields())
	aliceScore, bobScore := solution(A, B)
	write(fmt.Sprintf("%v %v\n", aliceScore, bobScore))
}

// SOLUTION ==============================================

func solution(A, B []int) (as, bs int) {
	for i, a := range A {
		b := B[i]
		if a > b {
			as++
